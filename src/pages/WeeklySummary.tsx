  ...
  const loadWeekData = async () => {
    ...
    try {
      ...
      const result: ServiceGroup[] = (services || []).map((service) => {
        ...
        // On cumule les quantités par (nom de produit + unité), pour éviter
        // de mélanger par exemple "1 kg" et "1 pièce" sous la même ligne.
        const lineMap = new Map<string, AggregatedLine>();
        for (const order of serviceOrders) {
          for (const item of order.order_items || []) {
            const unit = item.unit || "unité";
            const key = `${item.product_name}__${unit}`;
